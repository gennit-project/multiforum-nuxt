...
describe("Basic event operations", () => {
  ...
  it("creates, edits and deletes an online event with virtual event type", () => {
    ...
    cy.get('textarea[data-testid="description-input"]').type("Test description");
    ...
    cy.get('button[data-testid="event-menu-button').click();
    ...
    cy.get('input[data-testid="start-time-date-input"]')
      .clear()
      .type(formatDate(editedStartDateTime));

    // Change the start time
    cy.get('input[data-testid="start-time-time-input"]')
      .clear()
      .type(formatTime(editedStartDateTime));
