...
describe("Comment voting operations", () => {
  ...
  it("User 1 can undo upvote on their own comment", () => {
    ...
    cy.get('div[data-testid="comment"]').should("be.visible").and("contain", TEST_COMMENT_TEXT);
    ...
    cy.get('button[data-testid="upvote-comment-button"]').first().click({ force: true });
    ...
    cy.get('div[data-testid="comment"]')
      ...
      .within(() => {
        ...
        cy.get('button[data-testid="upvote-comment-button"]').should("be.visible").contains("1");
      });
  });
  ...
  it("User 2 can upvote another user's comment", () => {
    ...
    cy.get('button[data-testid="upvote-comment-button"]').first().contains("2");
    ...
    // Verify the count shows 1
    cy.get('button[data-testid="upvote-comment-button"]').first().contains("1");
  });
});
