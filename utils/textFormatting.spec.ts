...
describe('Text Formatting Utilities', () => {
  ...
  describe('Individual formatting functions', () => {
    ...
    describe('formatBold', () => {
      ...
      it('preserves whitespace', () => {
        expect(formatBold({ text: ' test with spaces ' })).toBe(
          '** test with spaces **'
        );
      });
    });
  ...
  describe('insertEmoji', () => {
    ...
    it('handles emoji insertion at beginning of text', () => {
      expect(insertEmoji({ text: 'Hello', position: 0, emoji: '👋' })).toBe(
        '👋Hello'
      );
    });
    ...
    it('handles multi-character emoji', () => {
      ...
      const familyEmoji = '👨‍👩‍👧‍👦';
      expect(
        insertEmoji({ text: 'Family: ', position: 8, emoji: familyEmoji })
      ).toBe('Family: 👨‍👩‍👧‍👦');
    });

    it('handles insertion between characters', () => {
      expect(
        insertEmoji({ text: 'HelloWorld', position: 5, emoji: '👋' })
      ).toBe('Hello👋World');
    });
  });
});
