...
Introduce one derived status for an installed `(plugin, version)` that everything
else renders or enforces. For each field the manifest declares (settings via
`ui.forms` / `settingsDefaults`, secrets via `secrets[]`):

| field kind | `required` | `isSet` | `isValid` |
|-----------|-----------|---------|-----------|
| setting | from field def | present in `settingsJson` (merged with defaults) | passes field validation (pattern/enum) |
| secret | from `secrets[]` | a `ServerSecret` exists for `(pluginId, key)` — **boolean only, never the value** | last-validation result (`VALID`/`INVALID`) |
...
- Backend: expose config status for an installed version — either extend the
  ...
  that returns per-field `{ key, kind, required, isSet, isValid }` (secrets as
  booleans). Reuse the existing `getServerPluginSecrets` logic for secrets and
  compute settings status from `settingsJson` + manifest field defs.
...
- **Orphaned secrets:** split the secrets list into "required by this version"
  ...
  latter in a separate "no longer used — remove?" affordance, not in the
  required list.
...
3. **Carry-over default:** default to carry-over-with-preview, or opt-in each
   upgrade? (Recommend default-on with preview + a "start fresh" escape.)
4. **Renamed secrets:** support manifest `renamedFrom` to re-point a
