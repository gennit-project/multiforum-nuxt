<script setup lang="ts">
  ...
  const eventWhere = computed(() => {
    return getEventWhere({
      ...
      channelId: channelId.value,
      onlineOnly:
        !channelId.value || filterValues.value.locationFilter === LocationFilterTypes.ONLY_VIRTUAL,
    });
  });
  ...
  const filterByTag = (tag: string) => {
    ...
    if (alreadySelected) {
      filterValues.value.tags = filterValues.value.tags?.filter((t: string) => t !== tag);
    } else {
