<script setup lang="ts">
  ...
  const handleClickTag = (tagText: string) => {
    ...
    if (currentQuery.tags) {
      ...
      } else if (Array.isArray(currentQuery.tags) && currentQuery.tags.includes(tagText)) {
        const newQuery = { ...route.query };
...
<template>
  <li
    :ref="`#${event.id}`"
    class="relative flex list-none justify-between gap-4 py-2"
    :data-testid="`event-list-item-${event.title}`"
    @click="handleClick"
  >
    <div class="flex-shrink-0 pt-2">
      <div class="flex w-16 flex-col items-center">
        <div class="font-semibold text-xs uppercase text-gray-500 dark:text-gray-200">
          {{
        ...
        <div class="font-semibold text-xs lowercase text-gray-500 dark:text-gray-200">
          {{
            new Date(event.startTime).toLocaleString("en-US", {
              month: "short",
            })
    ...
    <div class="min-w-0 flex-1">
      ...
      <div>
        <router-link
          class="text-md mt-2 flex cursor-pointer flex-wrap items-center gap-2 text-gray-800 hover:text-blue-700 dark:text-gray-200"
      ...
      <div class="flex gap-1">
        <div>
          ...
          >
            ...
            >
              ...
              <span class="text black text-sm dark:text-white">{{ `${commentCount}` }}</span>
              <span
                v-if="!isWithinChannel"
                class="ml-1 text-xs"
                >{{ `in c/${event.EventChannels[0].channelUniqueName}` }}</span
              >
            </button>
          </nuxt-link>
          ...
          >
            ...
            >
              ...
              {{
                ...
                } in ${channelCount} ${channelCount === 1 ? "channel" : "channels"}`
              }}
              <ChevronDownIcon
                aria-hidden="true"
        ...
        <div class="ml-auto mr-2 mt-2 hidden flex-shrink-0 rounded-lg md:flex">
          <img
            v-if="event.coverImageURL"
            :alt="event.title"
