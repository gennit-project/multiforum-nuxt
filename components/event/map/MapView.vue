<script setup lang="ts">
  ...
  import { chronologicalOrder, reverseChronologicalOrder } from "../list/filters/filterStrings";
  ...
  const showInPersonOnly = route.name === "map-search-eventId" || route.name === "map-search";
  ...
  const filterByChannel = (channel: string) => {
    ...
    if (alreadySelected) {
      ...
      filterValues.value.channels = filterValues.value.channels.filter((c) => c !== channel);
    } else {
      if (!filterValues.value.channels) {
  ...
  const highlightEventOnMap = (input: HighlightEventInput) => {
    const { eventId, eventLocationId, eventData, clickedMapMarker, markerMap, map } = input;
    if (eventId) {
      if (
    ...
    if (markerMap.markers[eventLocationId]) {
      ...
      const eventTitle = markerMap.markers[eventLocationId].events[highlightedEventId.value]?.title;
      const eventLocation =
        markerMap.markers[eventLocationId].events[highlightedEventId.value]?.locationName;
...
<template>
  <div class="flex flex-col">
    <client-only>
      ...
      >
        <div class="z-10 flex w-full justify-center bg-gray-100 dark:bg-gray-900">
      ...
      >
        ...
        <div class="fixed right-0 h-screen w-1/2 bg-gray-300 dark:bg-black">
          ...
          />
          <EventMap
            v-else-if="eventResult && eventResult.events && eventResult.events.length > 0"
