<script setup lang="ts">
  ...
  const loadMore = () => {
    fetchMore({
      variables: {
        options: {
          limit: DISCUSSION_PAGE_LIMIT,
          offset: discussionChannelResult.value?.getDiscussionsInChannel.discussionChannels.length,
          // @ts-ignore
  ...
  onMounted(() => {
    if (discussionChannelResult.value?.getDiscussionsInChannel.discussionChannels.length === 0) {
      refetchDiscussions();
    }
  });
  ...
  const reachedEndOfResults = computed(() => {
    return (
      discussionChannelResult.value?.getDiscussionsInChannel.aggregateDiscussionChannelsCount ===
      discussionChannelResult.value?.getDiscussionsInChannel?.discussionChannels.length
    );
  });
</script>
  ...
  <div class="px-2">
    ...
    >
      <ChannelDiscussionListItem
        v-for="discussionChannel in discussionChannelResult.getDiscussionsInChannel
          .discussionChannels"
    ...
    <div v-if="discussionChannelResult?.getDiscussionsInChannel?.discussionChannels?.length > 0">
      <LoadMore
