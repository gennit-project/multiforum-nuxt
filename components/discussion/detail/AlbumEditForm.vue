<script lang="ts" setup>
  ...
  import type { Discussion, Image, DiscussionUpdateInput } from "@/__generated__/graphql";
  ...
  const initialImageOrder = computed<string[]>(() => {
    ...
    if (images.value.length > 0) {
      ...
      return images.value.filter((img) => img.id).map((img) => img.id);
    }
  ...
  function getUpdateDiscussionInputForAlbum(): DiscussionUpdateInput {
    ...
    const imagesOps = [...connectImageArray, ...updateImageArray, ...deleteImageArray];
...
<template>
  <div class="w-full">
    <div class="mb-3 mt-3 flex w-full flex-col">
      <h3 class="font-semibold mb-4 text-lg dark:text-white">Edit Album (Optional)</h3>
      ...
      <div class="align-items mt-4 flex justify-end gap-2">
        <GenericButton
          :text="'Cancel'"
