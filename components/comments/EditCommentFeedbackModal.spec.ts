...
const mountModal = () =>
  mount(EditCommentFeedbackModal, {
    props: { commentId: 'c1', modName: 'mod1', open: true },
    global: {
      stubs: {
...
describe('EditCommentFeedbackModal', () => {
  it('shows the edit title', () => {
    const wrapper = mountModal();

    expect(modal(wrapper).props('title')).toBe('Edit your feedback');
  });
  ...
  it('submits the edit on the primary button', async () => {
    const wrapper = mountModal();
  ...
  it('shows an edit error banner', () => {
    ...
    expect(wrapper.find('.err').text()).toContain('edit boom');
  });
});
