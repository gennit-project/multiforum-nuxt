<script setup lang="ts">
  ...
  const toggleForumRuleSelection = (rule: string) => {
    if (selectedForumRules.value.includes(rule)) {
      selectedForumRules.value = selectedForumRules.value.filter((r) => r !== rule);
    } else {
  ...
  const toggleServerRuleSelection = (rule: string) => {
    if (selectedServerRules.value.includes(rule)) {
      selectedServerRules.value = selectedServerRules.value.filter((r) => r !== rule);
    } else {
...
<template>
  <GenericModal
    ...
    "
    :open="open"
    :primary-button-disabled="selectedForumRules.length === 0 && selectedServerRules.length === 0"
    ...
    <template #content>
      ...
      >
        ...
        <select
          ...
          class="block w-60 rounded-md border border-black p-2 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
        >
          <option
            disabled
            value=""
