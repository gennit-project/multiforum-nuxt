...
import { REPORT_CHANNEL } from '@/graphQLData/channel/mutations';
...
const {
  mutate: reportChannel,
  loading: reportChannelLoading,
  error: reportChannelError,
  onDone: reportChannelDone,
} = useMutation(REPORT_CHANNEL);
...
<template>
  <GenericModal
    data-testid="report-channel-modal"
    :highlight-color="'red'"
    :title="modalTitle"
    :body="'Why are you reporting this forum?'"
    :open="open"
    :primary-button-text="'Submit Report'"
    :secondary-button-text="'Cancel'"
    :loading="reportChannelLoading"
    :primary-button-disabled="selectedServerRules.length === 0"
    :error="reportChannelError?.message"
  ...
  >
    ...
    <template #content>
      ...
      <TextEditor
        test-id="report-channel-input"
        :initial-value="reportText"
        placeholder="Provide any additional context about why this forum should be reviewed"
      ...
      >
        ...
        server admins, not forum moderators.
      </div>
    </template>
  </GenericModal>
</template>
