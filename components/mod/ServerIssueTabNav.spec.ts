...
const mountNav = async (open: number, closed: number) => {
  mockedUseQuery
    .mockReturnValueOnce({
  ...
  return shallowMount(ServerIssueTabNav, {
    props: {
      openRouteName: 'admin-issues',
      closedRouteName: 'admin-issues-closed',
      createRouteName: 'admin-issues-create',
    },
    global: {
      stubs: {
...
describe('ServerIssueTabNav', () => {
  ...
  it('shows the open issue count from the aggregate', async () => {
    const wrapper = await mountNav(5, 3);
