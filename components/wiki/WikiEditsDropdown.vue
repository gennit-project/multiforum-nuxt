<script setup lang="ts">
  ...
  const allEdits = computed(() => {
    ...
    if (props.wikiPage?.PastVersions?.length) {
      ...
      const currentVersion = {
        ...
        title: props.wikiPage.title,
        Author: props.wikiPage.VersionAuthor,
      };

      // Add past versions
      props.wikiPage.PastVersions.forEach((version, index) => {
        const nextVersion = index === 0 ? currentVersion : props.wikiPage.PastVersions[index - 1];
  ...
  const handleRevisionDeleted = (deletedId) => {
    ...
    if (props.wikiPage?.PastVersions) {
      const index = props.wikiPage.PastVersions.findIndex((version) => version.id === deletedId);
      if (index !== -1) {
